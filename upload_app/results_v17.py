...
class results:
    ...
    def read_SUMMARY_csv(self, file):
        ...
        self.results_dict["Level_1_Total_Time"] = float(df["Level 1"].loc[0])
        self.results_dict["Level_2_Total_Time"] = float(df["Level 2"].loc[0])
        self.results_dict["Level_3_Total_Time"] = float(df["Level 3"].loc[0])
    ...
    def read_ProBookmarkRendering2D_log(self, file):
        ...
        for line in file.split("\n"):
            ...
            if line == "Bookmark, Map, DrawTime, AverageFPS, MinimumFPS, FPSSamples, TimeStamp":
                df = pd.read_csv(StringIO(file), sep=", ", skiprows=line_counter+2, names=line.split(", "), nrows=11)
    ...
    def read_ProEditing_log(self, file):
        ...
        for line in file.split("\n"):
            ...
            if line == "Bookmark, Map, DrawTime, AverageFPS, MinimumFPS, FPSSamples, TimeStamp":
                table_count += 1
                df = pd.read_csv(StringIO(file), sep=", ", skiprows=line_counter+2, names=line.split(", "), nrows=1)
